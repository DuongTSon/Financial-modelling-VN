...
try(download.file(htmlurl,tf))
rawhtml<-paste(readLines(tf,warn=FALSE),collapse="\n")
...
rate$date<-as.Date(rate$date,format="%m/%d/%Y")
...
if (max(data$date)<max(rate$date)){
  ...
  rate<-subset(final,final$date>=max(data$date))
  ...
  ## Raise rate changes by vector power
  ...
  index<-c(tail(data$neer,1)/100,index)
